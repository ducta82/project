...
$usp_post_meta_SubmitterPhone   = 'user_submit_phone';
...
function usp_checkForPublicSubmission() {
	...
	if (isset($_POST['user-submitted-post'], $_POST['usp-nonce']) && !empty($_POST['user-submitted-post']) && wp_verify_nonce($_POST['usp-nonce'], 'usp-nonce')) {
		...
		$author = ''; $url = ''; $email = ''; $tags = ''; $captcha = ''; $verify = ''; $content = ''; $category = '';$phone = '';
		...
		if (isset($_POST['user-submitted-phone']))    $phone  	= sanitize_text_field($_POST['user-submitted-phone']);
		...
		$result = usp_createPublicSubmission($title, $files, $ip, $author, $url, $phone, $email, $tags, $captcha, $verify, $content, $category);
...
function usp_add_meta_box() {
	...
	if (usp_is_public_submission()) {
		...
		$phone   = get_post_meta($post->ID, 'user_submit_phone', true);
		$url   = get_post_meta($post->ID, 'user_submit_url', true);
		$ip    = get_post_meta($post->ID, 'user_submit_ip', true); 
		
		if (!empty($name) || !empty($email) || !empty($phone) || !empty($url) || !empty($ip)) {
...
function usp_meta_box_callback($post) {
	...
	if (usp_is_public_submission()) {
		...
		if (!empty($name) || !empty($email) || !empty($url) || !empty($ip) || !empty($phone)) {
			...
			if (!empty($phone))   echo '<li>'. __('Submitter phone: ', 'usp')   . $phone  .'</li>';
...
// process submission
function usp_createPublicSubmission($title, $files, $ip, $author, $url, $phone, $email, $tags, $captcha, $verify, $content, $category) {
	global $usp_options, $usp_post_meta_IsSubmission, $usp_post_meta_SubmitterIp, $usp_post_meta_Submitter, $usp_post_meta_SubmitterUrl,$usp_post_meta_SubmitterPhone, $usp_post_meta_SubmitterEmail, $usp_post_meta_Image;
	...
	if (isset($usp_options['usp_phone'])      && ($usp_options['usp_phone']      == 'show') && empty($phone))      $newPost['error'][] = 'required-phone';
	...
	if ($newPost['id']) {
		...
		if (!empty($phone))    update_post_meta($post_id, $usp_post_meta_SubmitterPhone,   $phone);
...
function usp_error_message() {
	...
	if (isset($_GET['usp-error']) && !empty($_GET['usp-error'])) {
		...
		foreach ($error_array as $e) {
			...
			elseif ($e == 'required-phone')      $error[] = __('Post phone required', 'usp');
...
$usp_phone_display = array(
	...
	'disable' => array(
		'value' => 'disable',
		'label' => __('Do not auto-display submitted phone', 'usp')
	),
);
...
function usp_auto_display_options($item) {
	global $usp_options, $usp_image_display, $usp_email_display, $usp_url_display,$usp_phone_display;
...
function usp_validate_options($input) {
	global $usp_options, $usp_form_version, $usp_image_display, $usp_email_display, $usp_url_display, $usp_phone_display;
	...
	if (!isset($input['usp_use_phone'])) $input['usp_use_phone'] = null;
	$input['usp_use_phone'] = ($input['usp_use_phone'] == 1 ? 1 : 0);
						...
						<div class="toggle<?php if (!isset($_GET['settings-updated'])) { echo ' default-hidden'; } ?>">
							...
							<div class="mm-table-wrap mm-table-less-padding">
								<table class="widefat mm-table">
									...
									<tr>
										<th scope="row"><label class="description" for="usp_options[usp_phone]"><?php _e('Post Phone', 'usp'); ?></label></th>
										<td>
											<select name="usp_options[usp_phone]" id="usp_options[usp_phone]">
												<option <?php if ($usp_options['usp_phone'] == 'show') echo 'selected="selected"'; ?> value="show"><?php _e('Display and require', 'usp'); ?></option>
												<option <?php if ($usp_options['usp_phone'] == 'optn') echo 'selected="selected"'; ?> value="optn"><?php _e('Display but do not require', 'usp'); ?></option>
												<option <?php if ($usp_options['usp_phone'] == 'hide') echo 'selected="selected"'; ?> value="hide"><?php _e('Disable this field', 'usp'); ?></option>
											</select>
										</td>
									</tr>
									<tr>
							...
							<div class="mm-table-wrap">
								<table class="widefat mm-table">
									<tr>
										...
										<span class="mm-item-caption"><?php _e('Use registered username as post author. Valid when the person submitting the form is logged in to WordPress.', 'usp'); ?></span></td>
									</tr>
									<tr>
										...
										<span class="mm-item-caption"><?php _e('Use registered user&rsquo;s Profile URL as the post URL. Valid when the person submitting the form is logged in to WordPress.', 'usp'); ?></span></td>
									</tr>
									<tr>
