...
class WC_Points_Rewards_Order {
	...
	public function add_points_earned( $order ) {
		...
		// add order note
		$order->add_order_note( sprintf( __( 'Customer earned %d %s for purchase.', 'wc_advanced_points' ), $points, $wc_points_rewards->get_points_label( $points ) ) );
	}
	...
	public function maybe_deduct_redeemed_points( $order_id ) {
		...
		// add order note
		$order->add_order_note( sprintf( __( '%d %s redeemed for a %s discount.', 'wc_advanced_points' ), $points_redeemed, $wc_points_rewards->get_points_label( $points_redeemed ), woocommerce_price( $discount_amount ) ) );
	}
	...
	public function handle_cancelled_refunded_order( $order_id ) {
		...
		if ( $points_earned > 0 ) {
			...
			$points_total = (int)get_user_meta( $order->user_id, 'wc_points_level_user',true);
			update_user_meta( $order->user_id, 'wc_points_level_user', ($points_total - $points_earned) );

			// remove points from order
			delete_post_meta( $order->id, '_wc_points_earned' );

			// add order note
			$order->add_order_note( sprintf( __( '%d %s removed.', 'wc_advanced_points' ), $points_earned, $wc_points_rewards->get_points_label( $points_earned ) ) );
		}
		...
		if ( $points_redeemed > 0 ) {
			...
			// add order note
			$order->add_order_note( sprintf( __( '%d %s credited back to customer.', 'wc_advanced_points' ), $points_redeemed, $wc_points_rewards->get_points_label( $points_redeemed ) ) );
		}
	}
