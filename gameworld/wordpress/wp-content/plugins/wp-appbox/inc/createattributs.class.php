...
class wpAppbox_CreateAttributs {
	...
	*/
	...
	function isValueStore( $value ) {
		...
		if ( 'windowsphone' == $value ) $value = 'windowsstore';
		if ( array_key_exists( $value, $wpAppbox_storeNames ) ) {
			return( true );
		} else {
			return( false );
		}
	...
	/**
	* Prüfen ob Shortcode-Attribut "noQRcode" ist [deprecated]
	*
	* @since   2.0.0
	* @remove  >3.x.x
	*
	* @param   string   $value      Attribut
	* @return  boolean  true/false  TRUE when "noQRcode"
	*/
	...
	*/
	...
	function devideAttributs( $attribute ) {
		...
		if ( $alterpreis || $oldprice ) {
			$attr['oldprice'] = $alterpreis.$oldprice;
		}
