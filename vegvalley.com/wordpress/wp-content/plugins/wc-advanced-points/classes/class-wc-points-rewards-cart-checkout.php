...
class WC_Points_Rewards_Cart_Checkout {
	...
	public function coupon_label( $label ) {
		if ( strstr( strtoupper( $label ), 'WC_POINTS_REDEMPTION' ) ) {
			$label = esc_html( __( 'Points redemption', 'wc_advanced_points' ) );
		}
	...
	public function render_redeem_points_message() {
		...
		$message .= '<input type="submit" class="button wc_points_rewards_apply_discount" name="wc_points_rewards_apply_discount" value="' . __( 'Apply Discount', 'wc_advanced_points' ) . '" /></form>';
		...
		if ( 'yes' === get_option( 'wc_points_rewards_partial_redemption_enabled' ) ) {
			...
			$js = '
				$( "input.wc_points_rewards_apply_discount" ).click( function() {
					var points = prompt( "' . esc_js( __( 'How many points would you like to apply?', 'wc_advanced_points' ) ) . '", "' . $points . '" );
					if ( points != null ) {
